...
class NEMProcessorTest {
    ...
    void processNMIBaseInfo() {
        ...
        for (String row : errorTestData) {
            String[] columns = row.split(",");
            NEMErrorWriter errorWriter = factory.createNEMErrorWriter("");
            processor.processNMIBaseInfo(columns, new RuntimeState(), errorWriter);
    ...
    static class NEMCheckpointWriterMock implements NEMCheckpointWriter {
        private List<RuntimeState> checkpoints = new ArrayList<>();

        @Override
        public RuntimeState getStartingState() {
            return checkpoints.isEmpty() ? new RuntimeState() : checkpoints.get(checkpoints.size() - 1);
        }

        @Override
        public void flush(RuntimeState state) {
            checkpoints.add(new RuntimeState(state.nmi, state.interval, state.inputFileName, state.currentLineCnt));
        }
